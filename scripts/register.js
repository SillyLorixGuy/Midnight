...
// Step 1: Email + Password → signUp → email confirmation
// Step 2: After email confirmed → complete profile (username, bio, pfp)
...
// Check on page load if user is already confirmed and needs to complete profile
(async function checkInitialState() {
  ...
  if (user) {
    ...
    if (error || !data || !data.username) {
      // Profile incomplete - show step 2
      showStep2();
    } else {
...
step2Btn.addEventListener('click', async (e) => {
  ...
  try {
    ...
    // Update or insert profile (convert visibility string to boolean)
